...
CREATE TABLE IF NOT EXISTS relations (
    id BIGSERIAL PRIMARY KEY,
    document_id BIGINT NOT NULL REFERENCES documents(id) ON DELETE CASCADE,
    head_entity_id BIGINT NOT NULL REFERENCES entities(id) ON DELETE CASCADE,
    tail_entity_id BIGINT NOT NULL REFERENCES entities(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS annotation_task_documents (
    id BIGSERIAL PRIMARY KEY,
    annotation_task_id BIGINT NOT NULL REFERENCES annotation_tasks(id) ON DELETE CASCADE,
    document_id BIGINT NOT NULL REFERENCES documents(id) ON DELETE CASCADE,
    status VARCHAR(20) DEFAULT 'pending',
